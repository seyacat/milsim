...
import { getControlPointById } from './control-point-operations.js';
...
export function updatePositionChallengeBars(controlPointId, teamPoints) {
  ...
  if (!map) {
    console.error('Map not available for updating position challenge bars');
    return;
  }
...
export function createOwnerPositionChallengePieChart(marker, controlPoint, positionCircle) {
    const map = getMap();
    
    if (!map) {
        console.error('Map not available for creating position challenge pie chart');
        return;
    }

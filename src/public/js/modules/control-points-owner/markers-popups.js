...
import { createOwnerPositionChallengePieChart } from './position-challenge.js';
import { requestControlPointData } from './control-point-operations.js';
import { updateBombButtonStates } from './bomb-timer.js';
import { createControlPointEditMenu } from './edit-menu.js';
import { mapClickHandler } from './state.js';
import { getMap, getCurrentGame } from '../game-core/index.js';

// Add control point marker to map for owners
export function addControlPointMarkerOwner(controlPoint) {
    const map = getMap();
    const currentGame = getCurrentGame();
    
    if (!map) {
        console.error('Map not available for adding control point marker');
        return null;
    }
    ...
    // Create the popup content immediately with the current control point data
    console.log('Creating popup for control point:', controlPoint.id, controlPoint.name);
    const popupContent = createControlPointEditMenu(controlPoint, marker);
    
    if (!popupContent) {
        console.error('Failed to create popup content for control point:', controlPoint.id);
        return marker;
    }
    ...
    console.log('Popup created successfully for control point:', controlPoint.id);
    ...
    marker.on('popupopen', function() {
        ...
        // Request fresh control point data when popup opens to update with latest state
...
export function updateOwnerControlPointPopups() {
    const map = getMap();
    if (!map) return;
...
export function refreshOwnerControlPointMarkers(controlPoints) {
    const map = getMap();
    if (!map) return;
...
export function updateSingleOwnerMarker(controlPoint) {
    const map = getMap();
    if (!map) return;
...
export function hasSiteControlPoint() {
    const map = getMap();
    if (!map) return false;

...
import { requestPlayerControlPointData } from './challenge-submissions.js';
import { activeBombTimers } from './state.js';

// Create control point interaction menu for players
export function createControlPointPlayerMenu(controlPoint, marker) {
    ...
    if (controlPoint.ownedByTeam) {
        ...
        ownershipStatus = `
            <div class="ownership-status" style="color: ${controlPoint.ownedByTeam}; font-weight: bold;">
                Controlado por: ${TEAM_NAMES[controlPoint.ownedByTeam] || controlPoint.ownedByTeam}
            </div>
...
export function addControlPointMarkerPlayer(controlPoint) {
    ...
    if (controlPoint.ownedByTeam) {
        iconColor = TEAM_COLORS[controlPoint.ownedByTeam] || '#2196F3';
    } else {
...
export function updateControlPointPopups() {
    if (!map) return;
...
export function updateControlPointPopupForBomb(controlPointId) {
    if (!map) return;
...
export function refreshControlPointMarkers(controlPoints) {
    if (!map) return;
...
export function updateTimerDisplay(controlPointId, ownedByTeam, displayTime) {
...
export function updatePlayerControlPointPopupWithFreshData(controlPointId, marker) {
...
export function createPositionChallengePieChart(marker, controlPoint, positionCircle) {
    ...
    // Store SVG overlay reference on marker
    marker.pieSvg = svgOverlay;
    marker.pieSvgElement = svgElement;
    
    // Return the SVG element for later updates
    return svgElement;
}

// Update position challenge pie chart with team points
export function updatePositionChallengePieChart(controlPointId, teamPoints) {
    if (!map) return;
    
    // Find the marker with this control point
    ...
    if (!targetMarker || !targetMarker.pieSvgElement) return;
    
    const svgElement = targetMarker.pieSvgElement;
    
    // Clear existing pie chart content
    ...
    if (totalPoints === 0) {
        // No points yet - show empty pie chart
        const emptyCircle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        emptyCircle.setAttribute('cx', svgElement.getAttribute('width') / 2);
        emptyCircle.setAttribute('cy', svgElement.getAttribute('height') / 2);
        emptyCircle.setAttribute('r', svgElement.getAttribute('width') / 2 - 5);
        emptyCircle.setAttribute('fill', 'rgba(255, 152, 0, 0.1)');
        emptyCircle.setAttribute('stroke', '#FF9800');
        emptyCircle.setAttribute('stroke-width', '2');
        svgElement.appendChild(emptyCircle);
        return;
    }
    
    // Create pie chart segments
    let startAngle = 0;
    const centerX = svgElement.getAttribute('width') / 2;
    const centerY = svgElement.getAttribute('height') / 2;
    const radius = svgElement.getAttribute('width') / 2 - 5;
    ...
    Object.entries(teamPoints).forEach(([team, points]) => {
        if (points > 0) {
            ...
            path.setAttribute('fill', TEAM_COLORS[team] || '#666666');
...
// Update position challenge bars display
export function updatePositionChallengeBars(controlPointId, teamPoints) {
    ...
    if (totalPoints === 0) {
        // No points yet - show empty state
    ...
    Object.entries(teamPoints).forEach(([team, points]) => {
        if (points > 0) {
            ...
            bar.style.cssText = `
                height: 8px;
                background: ${TEAM_COLORS[team] || '#666666'};
                border-radius: 2px;
                width: ${percentage}%;
                min-width: 2px;
            `;
...
window.submitCodeChallenge = function(controlPointId) {
    ...
    // Submit code challenge via WebSocket
...
window.submitBombChallenge = function(controlPointId) {
    ...
    // Submit bomb challenge via WebSocket
    if (socket && socket.readyState === WebSocket.OPEN) {
        socket.send(JSON.stringify({
...
window.submitBombDeactivation = function(controlPointId) {
    ...
    // Submit bomb deactivation via WebSocket
    if (socket && socket.readyState === WebSocket.OPEN) {
        socket.send(JSON.stringify({

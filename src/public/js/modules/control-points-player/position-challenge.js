...
import { getControlPointById } from './challenge-submissions.js';
import { createPositionChallengePieChart } from './markers-popups.js';
  ...
  if (totalPoints > 0 && controlPoint && controlPoint.hasPositionChallenge && currentGame && currentGame.status === 'running') {
    ...
    Object.entries(teamPoints).forEach(([team, points]) => {
      if (points > 0) {
        ...
        path.setAttribute('fill', TEAM_COLORS[team] || '#9E9E9E');
...
export function handlePositionChallengeUpdate(data) {
...
export function updateAllPositionChallengeBars() {
...
export function startPositionChallenge() {
    // This function is called when the game starts running
    // The actual interval is managed in the main index.js file
}

// Stop position challenge interval
export function stopPositionChallenge() {
    // This function is called when the game stops or pauses
    // The actual interval is managed in the main index.js file
}
